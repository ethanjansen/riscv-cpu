...
As per RV32I specification, data memory will support storing/loading halfwords and bytes.
...
Register x0 is writable but is always holds zeros and the other generic registers (x1-x31) function properly.
This implementation has been tested using the VHDL testbenches built into ISE.
Tests include reading and writing to registers, reading and writing to register x0, and writing to and reading from the same register.
The register block is implemented using a write-first memory block which is an array of \verb|std_logic_vector|s, similar to how the memory is implemented.
...
In addition to arithmetic operations, the ALU performs comparisons between the two inputs, which are used for branch conditions and for SLTI and SLTIU.
...
One might expect full coverage testing of the flags to require $2^{4}$ tests but, since the flags are not independant, the total required tests drops to five.
